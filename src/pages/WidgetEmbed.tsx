...
const WidgetEmbed = () => {
  ...
  const trackApiCall = async (callType: string, status: string = "success") => {
    if (!widgetId || !widgetSettings?.user_id) return;

    try {
      await supabase.from("widget_api_calls").insert({
        widget_id: widgetId,
  ...
  const handleScan = async () => {
    ...
    setScanning(true);
    try {
      // Here you would call your actual food scanning API
