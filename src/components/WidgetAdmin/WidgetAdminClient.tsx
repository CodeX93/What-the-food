...
export function WidgetAdminClient() {
  ...
  const [overviewStats, setOverviewStats] = useState<WidgetAdminStats | null>(null);
  const [userList, setUserList] = useState<WidgetAdminUser[]>([]);
  const [apiCalls, setApiCalls] = useState<any[]>([]);
  const [topUsers, setTopUsers] = useState<WidgetAdminUser[]>([]);

const loadData = useCallback(async () => {
    try {
      const { data: allUsers } = await (supabase as any).from("profiles").select("id, email");
      const { data: allSubscriptions } = await (supabase as any).from("widget_subscriptions").select("*");
      const { data: allApiCalls } = await (supabase as any)
        .from("widget_api_calls")
        .select("*")
        .order("created_at", { ascending: false });
      const { data: allSites } = await supabase.from("widget_sites").select("*");

      const today = new Date().toDateString();
      const todayCalls = allApiCalls?.filter((call) => new Date(call.created_at).toDateString() === today) || [];

      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);
      const weekCalls = allApiCalls?.filter((call) => new Date(call.created_at) >= weekAgo) || [];

      const monthAgo = new Date();
      monthAgo.setMonth(monthAgo.getMonth() - 1);
      const monthCalls = allApiCalls?.filter((call) => new Date(call.created_at) >= monthAgo) || [];
      ...
      const usersWithSubs: WidgetAdminUser[] = await Promise.all(
        ((allUsers || []) as Array<{ id: string; email?: string | null }>).map(async (profile) => {
          ...
          const { data: userApiCalls } = await (supabase as any)
            .from("widget_api_calls")
            .select("*")
            .eq("user_id", profile.id);
      ...
      setTopUsers(usersWithSubs.slice().sort((a, b) => b.apiCallsCount - a.apiCallsCount).slice(0, 10));
...
useEffect(() => {
  ...
  const filteredUsers = userList.filter((user) => {
    ...
    return (
      ...
      user.subscription?.subscription_type?.toLowerCase().includes(lower)
    );
  });
    ...
    <main className="flex-1">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Track user performance and API consumption</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                <Zap className="h-4 w-4" /> Total API Calls
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{overviewStats?.totalApiCalls || 0}</div>
              <p className="text-xs text-muted-foreground mt-1">
                {overviewStats?.successfulCalls || 0} successful, {overviewStats?.errorCalls || 0} errors
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                <Activity className="h-4 w-4" /> Today&apos;s Calls
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                <BarChart3 className="h-4 w-4" /> Total Sites
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{overviewStats?.totalSites || 0}</div>
              <p className="text-xs text-muted-foreground mt-1">Widget sites registered</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="users" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="api-calls">
              <Zap className="h-4 w-4 mr-2" /> API Calls
            </TabsTrigger>
          ...
          <TabsContent value="users" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>View and manage all widget users and their subscriptions</CardDescription>
              </CardHeader>
                      ...
                      <div key={user.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex-1">
                          ...
                          <div className="flex gap-4 mt-1 text-sm text-muted-foreground">
                            <span>
                              Plan: <span className="font-medium">{user.subscription?.subscription_type || "N/A"}</span>
                            </span>
          ...
          <TabsContent value="api-calls" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Monitor all API calls from widgets</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex gap-2">
                  <Button
                    ...
                    filteredApiCalls.map((call) => (
                      <div key={call.id} className="flex items-center justify-between p-3 border rounded-lg text-sm">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            {call.site_url || "Unknown site"} • {new Date(call.created_at).toLocaleString()}
                          </p>
                        </div>
                        {call.response_time_ms && (
                          <span className="text-xs text-muted-foreground">{call.response_time_ms}ms</span>
                        )}
                      </div>
                    ))
                  )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Users with the most API consumption</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {Object.entries(
                      userList.reduce<Record<string, number>>((acc, user) => {
